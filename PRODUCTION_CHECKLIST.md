...
- [x] Database migration from local MinIO-style self-hosted Postgres to AWS RDS, replacing it everywhere including local dev (not just production) — same "prove it for real" philosophy already used for S3. Started 2026-09-12, closed 2026-09-12.

  **AWS side:**
  - Instance `spark-primary-db` (ap-south-2c), engine PostgreSQL 15.19, class `db.m7g.xlarge` (4 vCPU / 16 GiB, Graviton3 — chosen over `m6g`/`m5`/`m6i` after a real price/performance audit across the whole `m` family; `m7i` and AMD variants ruled out on availability/non-existence grounds, not preference), 100 GiB gp3 with storage autoscaling (1000 GiB ceiling), Single-AZ (deliberate cost/safety tradeoff, made explicitly against the "never compromise safety for cost" default — see the sizing discussion for the full risk/mitigation table), no read replica (same single-server decision, reversed once from an initial "1 dual-purpose replica" plan), automated backups enabled (14-day retention), deletion protection on, maintenance window pinned to Sunday 02:00–03:00 UTC (not left to "no preference", so a routine reboot can never land mid-exam), Enhanced Monitoring + Database Insights (Standard tier — Advanced's fleet-level monitoring has no value for one instance) enabled.
  - Public access enabled (temporarily, same reasoning as S3): app compute isn't on AWS yet, so local dev needs a real path to RDS. Locked down via a dedicated security group (`spark-rds-sg`) allowing only the developer's actual outbound IP on port 5432 — found and fixed a real mismatch mid-migration where the browser's IP differed from Docker's own egress IP on the same machine.
  - `rds.force_ssl = 1` set via a custom parameter group (`spark-postgres15-force-ssl`, dynamic parameter, applied immediately, no reboot) — closes a real gap where only the client (PgBouncer) was requiring TLS; the server itself would have silently accepted a plaintext connection if that client setting were ever reverted. Verified directly: a `sslmode=require` connection succeeds, a `sslmode=disable` connection is rejected outright.

  **Code side:**
  - Per-service databases (`auth_db`, `user_db`, `resource_db`, `practice_db`, `assessment_db`, `notification_db`, `analytics_db`) and their dedicated least-privilege users created on RDS, replicating `infra/init-db.sql`'s exact permission model (REVOKE PUBLIC connect, per-user GRANT scoped to their own database only, default privileges for future migrations) — not the master user for app traffic.
  - All ~65MB of existing dev data migrated via `pg_dump | psql` per database; table counts verified matching exactly on both sides for all 7 databases, sequence values (auto-increment counters) preserved correctly.
  - `infra/pgbouncer/pgbouncer.ini` (shared between `docker-compose.dev.yml` and `docker-compose.prod.yml`) repointed at the RDS endpoint — this one file change fixed the actual data-plane routing for both dev and prod simultaneously. `server_tls_sslmode = require` added so the PgBouncer→RDS hop itself is encrypted.
  - `docker-compose.dev.yml`: removed the local `postgres` and `db-backup` services and the `postgres_data`/`backup_data` volumes (RDS's own automated backups now cover what `db-backup`'s cron `pg_dump` used to do — running that job on the same EC2 host as the live app services later would only reintroduce resource contention with them, and a same-host backup isn't real redundancy anyway). Two services (`notification-beat`, `analytics-service`) that depended directly on local `postgres` switched to depending on `pgbouncer` instead.
  - `docker-compose.prod.yml`: same treatment — removed its own separate local `postgres`/`db-backup` service definitions (this file was not touched during the S3 migration since it never had its own MinIO block, but it did have a real, separate self-hosted Postgres setup that needed the same fix) and redirected all 14 app services' `depends_on: postgres` to `depends_on: pgbouncer`. Found and fixed two additional stale leftovers while in this file: `nginx`'s `9002:9002` port mapping in both dev and prod compose files (the MinIO CORS proxy it served was already removed from `nginx.conf`/`nginx.dev.conf` during the S3 migration, but the port mapping itself was never cleaned up).
  - Full end-to-end verification: real create/read/delete CRUD through the actual HTTP stack (gateway → service → PgBouncer → RDS) across two different databases (`auth_db`, `resource_db`), not just container health checks. Re-verified again after the `db-backup`/`postgres` removal and after the `rds.force_ssl` change — all passing.
  - `infra/init-db.sql` and `infra/scripts/backup.sh` are no longer referenced by either compose file post-migration — left in place rather than deleted, since `init-db.sql`'s exact GRANT/REVOKE logic was directly reused to set up RDS's own per-service users, and remains the canonical reference for that permission model.

  **Still open:**
  1. Backup restore was never actually tested — RDS is confirmed *taking* automated backups, but nobody has verified a *restore* actually works. Deliberately deferred (not forgotten) — matters most given Single-AZ + no replica means these backups are the real safety net, not a formality.
  2. The RDS master password was inadvertently displayed in plaintext (twice) via terminal output pasted into chat during connection testing. Same category of incident as the S3 access key exposure — the owner was informed and explicitly declined rotation, accepting the risk knowingly.

- [x] Token-refresh rate limiting fixed end-to-end — a student sharing an exam-hall/lab IP with other students could get force-logged-out mid-exam by a per-IP rate limit, with their `refresh_token` never actually being invalid. Reported as a live production concern (not discovered via testing) and fixed same-day. Started/closed 2026-09-12.

  **The bug (three layers deep, all three had to be found and fixed together):**
  1. Access tokens live 15 min (`services/auth-service/core/settings.py` `SIMPLE_JWT`) — every logged-in browser calls `POST /api/auth/token/refresh/` automatically roughly every <15 min for the length of an exam.
  2. `gateway/nginx.conf`'s `auth_zone` (5r/m, sized for login brute-force protection) was shared by `/api/auth/token/` — including refresh — so a handful of students behind one shared public IP (school NAT/gateway), most starting the exam within the same few minutes, was enough to exceed it.
  3. `frontend/src/lib/api.ts`'s refresh-on-401 interceptor treated *any* refresh failure — a genuine invalid/blacklisted token, a network blip, or this exact 429 — identically: immediate `forceLogout()`, clearing the session and hard-redirecting to login, even though the refresh token itself was completely valid.

  **Fix — nginx layer:**
  - Added a dedicated `token_refresh_zone` (`gateway/nginx.conf`: 120r/m burst 60; `nginx.dev.conf`: 200r/m burst 100), split out from `auth_zone`, which stays at 5r/m for login/logout — brute-force protection on credentials is unchanged. A refresh token isn't a guessable secret, so brute-force was never the threat model for this endpoint; the new ceiling exists to keep pace with legitimate concurrent exam traffic. `Retry-After` map updated accordingly. Verified with a real `nginx -t` syntax check on both files.

  **Fix — Django layer (found during audit, not part of the original report):** raising nginx's ceiling alone was not sufficient — `TokenRefreshView` (`rest_framework_simplejwt`, used as-is in `urls.py`) fell through to the service's global `DEFAULT_THROTTLE_RATES["anon"]` (60/min per IP), silently capping the endpoint *beneath* nginx's new 120/min regardless of the nginx fix. Added `TokenRefreshThrottle` (`authentication/throttling.py`, scope `token_refresh`, 120/min — kept at parity with nginx rather than raised further) and wired it via a thin `TokenRefreshView` subclass (`authentication/views.py`) rather than editing the third-party view or loosening the global anon default for every other endpoint.

  **Fix — frontend layer (`frontend/src/lib/api.ts`):** the refresh-on-401 interceptor now retries up to 3 times specifically on a 429, honoring the gateway's `Retry-After` header when present (1s/2s backoff otherwise) — `forceLogout()` is now reserved for an actual 400/401 "invalid or blacklisted" response. A 429 that survives all retries fails that one request softly instead of clearing the session.

  **Fix — offline answer queue (found during audit, not part of the original report):** `frontend/src/lib/assessmentOfflineQueue.ts`'s `isRetryableNetworkError()` only queued a failed answer-save for retry on a transport-level failure (no HTTP response at all) — a 429 *is* a real response, so even after the interceptor fix above stopped it from logging the student out, a rate-limited refresh feeding into an in-flight answer-save would still throw as a "genuine rejection" and the answer would be dropped rather than queued. Fixed to treat 429 the same as a network drop (retryable), while genuine 4xx/5xx rejections (403 time-expired, etc.) still throw as before — this was the fix that actually closes the loop on "the student's work is never lost."

  **Verification:** 5 new frontend tests added (`api.test.ts`: 429-doesn't-logout, retry-then-succeed, genuine-400-still-logs-out; `assessmentOfflineQueue.test.ts`: 429-queues-on-save, 429-stays-queued-on-flush) — full frontend suite 373/373 passing. 3 new backend tests added (`test_refresh.py`: URL resolves to the subclassed view, view uses `TokenRefreshThrottle` not the global anon default, throttle rate is actually 120/min not silently 60/min) — closes the gap where the Django-layer fix had no regression coverage of its own wiring. Auth-service's full suite (248 tests) run inside its actual Docker image — not a local venv, to match how CI runs it — 248/248 passing, twice consecutively per this repo's 200% rule. Both `docker compose config --quiet` invocations (dev+integration-test, prod) still validate after the gateway config changes.

  **Deliberately not changed (flagged, not silently omitted):**
  1. Multi-tab session duplication (a student copies their exam tab, both tabs share one refresh token) still logs out whichever tab loses the rotation race — inherent to JWT rotation plus the deliberate per-tab session architecture (`frontend/AGENTS.md`, 2026-08-27), not the shared-IP scenario this fix addresses, and not worth engineering around for a rare, self-inflicted edge case.
  2. `limit_req_zone` keys on `$binary_remote_addr` — correct today (client connects directly to the EC2 instance, no load balancer/CDN in front yet). If one is added later, this needs `real_ip_header`/trusted-proxy config, or every client behind it collapses into one IP and rate limiting breaks entirely. Revisit when the compute migration adds a load balancer.

- [x] ClamAV `mem_limit` raised in both compose files — the container could get stuck indefinitely (healthcheck failing, never recovering) during a routine `freshclam` virus-signature database reload. Found live in dev, not from testing: `infra-clamav-1` sat "unhealthy" for 24+ hours, pinned at ~99.8% of its 1.5g memory limit (confirmed via `docker inspect`/`docker stats`, not assumed) — a reload briefly holds the OLD signature set in memory while validating and loading the NEW one before the atomic swap, and 1.5g (previously sized only for steady-state load, see the comment already in both compose files) leaves no headroom for that spike. This is not dev-specific — `freshclam`'s daily reload runs identically in production, on the same image with the same limit, so it would have eventually hung there too. `docker-compose.dev.yml`: raised to `2200m` (confirmed to fit within this host's existing ~3.8GB Docker budget — every other container combined measured at ~1.4GB, so no need to also raise Docker Desktop's own memory allocation). `docker-compose.prod.yml`: raised to `3000m` (the EC2 instance sizing, `m6g.xlarge`/16GB, already budgeted comfortable headroom for this). Fixed live by recreating the container with the new limit; confirmed it completed the in-progress reload and passed its healthcheck. Started/closed 2026-09-15.

- [x] Two more gaps found while testing the app locally via `npm run dev` (frontend outside Docker, hitting the dockerized backend through nginx) — a scenario that had never actually been exercised before, since the frontend always ran through the same nginx container until now. Started/closed 2026-09-15.

  1. **`frontend/.env.local`** (local-only, not committed) had `NEXT_PUBLIC_API_URL=http://localhost:3000/api` — pointed the frontend at its own dev-server port instead of nginx (port 80), so every API call 404'd against Next.js itself. Corrected to `http://localhost/api`.
  2. **CORS was never configured in `gateway/nginx.dev.conf` at all** — never needed before, since the frontend and API always shared one origin (`http://localhost`) via the same nginx container. Running the frontend standalone on `:3000` makes it a genuinely different origin from the API on `:80` for the first time. Added a dev-only `$cors_origin` map (`localhost:3000`/`127.0.0.1:3000` only — fails closed, mirrors `nginx.conf`'s prod pattern for the real Vercel domain) and the matching CORS headers.
  3. **Found and fixed a real, pre-existing bug surfaced by testing #2 live**: the OPTIONS-preflight handler in both nginx configs used a bare `if ($request_method = OPTIONS) { return 204; }` directly in `server` context. Confirmed via live `curl` that this does **not** reliably short-circuit before a location's own `proxy_pass` — even the simplest endpoint (`/api/auth/health/`, no overrides) proxied the OPTIONS request straight through to Django instead of getting nginx's 204. This means the *production* CORS fix (`gateway/nginx.conf`, added earlier the same day) likely never actually answered a real preflight from the live Vercel frontend either — found only because dev's identical pattern was tested live for the first time; would not have been caught by `nginx -t` (config is syntactically valid) or by `docker compose config` (doesn't test runtime behavior). Fixed in **both** `nginx.conf` and `nginx.dev.conf` using the same `error_page` redirect trick already proven working elsewhere in these files (the 429 rate-limit handler) — `if` returns a distinct status (418, otherwise unused) and `error_page 418 = @cors_preflight;` routes it to a named location that sends the real 204. Verified live via direct `curl -X OPTIONS`: now returns 204 with correct `Access-Control-Allow-*` headers. Also had to `nginx -s reload` (later `docker restart`) after each config edit — the bind-mounted file updates immediately, but the running nginx worker doesn't pick it up until reloaded; `docker compose up -d nginx` alone does not force this when the container itself isn't recreated.

- [x] Login security audit — all 4 login screens (student, admin, super_admin, it) share one backend endpoint (`authentication/views.py::LoginView`) and one rate-limit config, so this audit and its fixes cover all four at once. Started/closed 2026-09-15.

  **Already solid, no change needed:**
  - **SQL injection**: not a concern — grepped the whole service for `raw()`/`.extra()`/`cursor.execute()`; none exist. Every query goes through the Django ORM's parameterized queries.
  - **Rate limiting**: correctly layered defense-in-depth, and deliberately left untouched — unlike token refresh (fixed earlier the same day), login genuinely needs to stay tight: nginx's `auth_zone` (5r/m per IP) plus a separate per-account `LoginAttemptThrottle` (5/min, keyed on the submitted `student_id`/email, not IP) — the two protect against different attack shapes (one attacker spreading guesses across many IPs vs. many IPs hammering one account) and neither substitutes for the other.
  - **Error-message content**: already correctly generic and non-enumerating — unknown student_id, unknown email+role, a data-integrity edge case (`MultipleObjectsReturned`), and a genuinely wrong password all return the exact same `"Invalid credentials"` / 401, so the response body itself never reveals whether an account exists.
  - **Session invalidation**: `TokenVersionJWTAuthentication` checks a per-user `token_version` against the JWT's own copy on every request — a password change immediately invalidates every previously issued token, not just at expiry.
  - **Password storage**: Django's default hasher chain (PBKDF2, no downgrade in `core/settings.py`) — not Argon2 (not installed), but this is still an industry-accepted, NIST-approved choice, not a gap requiring action.
  - **CSRF**: not applicable — `DEFAULT_AUTHENTICATION_CLASSES` is JWT-only (`TokenVersionJWTAuthentication`), no `SessionAuthentication`/cookie-based auth in the mix, so there's no CSRF-vulnerable surface here to protect in the first place.

  **Found and fixed:**
  1. **Timing-attack account enumeration** (`authentication/views.py::LoginView`) — the error *message* never leaked whether an account exists, but response *time* did: the `User.DoesNotExist` branch returned immediately, while an existing account always ran a real password hash comparison (deliberately slow, by design, via PBKDF2) — measurably slower. An attacker could enumerate valid `student_id`/email+role combinations purely from timing, without ever seeing a different error string. This is exactly the class of bug Django's own built-in `ModelBackend` already defends against (its own code comments cite Django ticket #20760) — this codebase's login bypasses that built-in protection by querying the user manually instead of going through `authenticate()`. Fixed by mirroring Django's own idiom: call `User().set_password(password)` on the `DoesNotExist` path to burn equivalent CPU time before responding, for both the student_id and email branches. Verified: `test_login.py`/`test_refresh.py` (16 tests) still pass unchanged, in the real Docker image.
  2. **Raw/unhelpful error messages surfaced to the user — the priority item, fixed globally, not per-screen.** None of the 4 login screens handled a 429 (rate-limited) response with a clear message. nginx's 429 body (`{"error": "...", "retry_after": "..."}`) has no `message`/`detail` field, so `getErrorMessage()` fell through every check to axios's generic `"Request failed with status code 429"` — shown to the user *verbatim* on 3 of 4 screens (admin/super_admin/it just render `getErrorMessage()` directly), and on the student screen too since its keyword-based remapping doesn't recognize that string either and falls through to its own raw-message fallback. This is exactly the scenario already fixed for token refresh earlier the same day — students/admins sharing one exam-hall/lab IP colliding on the 5/min login rate limit at the start of an exam would see a confusing technical string instead of an actionable message.

     Fixed centrally in `getErrorMessage()` (`frontend/src/lib/api.ts`) rather than in each of the 4 forms, and then generalized further (deliberately, per an explicit "this must be global, not just login" instruction): the *same* fallthrough problem existed for every other unrecognized response shape too, not just 429 — a genuine 500/502, or any body without `message`/`detail`/`non_field_errors`/a field-error array, still fell through to axios's raw `"Request failed with status code NNN"`. Both `PortalLoginForm.tsx` (admin/super_admin/it) and `students/login/page.tsx` already funnel every error through this one shared function, so hardening it once — 429 gets `"Too many attempts..."` (with the server's `Retry-After` seconds when present), 5xx gets `"Something went wrong on our end. Please try again shortly."`, anything else unrecognized gets `"Something went wrong. Please try again."` — fixes all 4 screens (and every other caller of this helper across the app) at once, by construction, rather than requiring 4 separate patches that could drift out of sync later. 4 new tests added (2 for the 429 case, 2 for the generic 500/unrecognized-shape fallback), full suite 379/379 passing, twice consecutively.

  **Deliberately not changed (flagged, not silently omitted):**
  - No CAPTCHA and no additional account-lockout mechanism beyond the two rate limits above. Accepted as sufficient for the current scale and threat model (an internal institutional tool, not a public consumer app) — adding either would be meaningful extra complexity (CAPTCHA needs a third-party service + frontend integration; lockout needs an unlock flow, or it becomes its own DoS vector) for marginal benefit over the rate limiting already in place. Revisit if real credential-stuffing activity is ever observed.
  - No MFA on the admin/super_admin/it logins, despite being the most privileged roles. A real gap for a genuinely public-facing production system, but out of scope to add speculatively here without being asked — flagging it explicitly rather than either silently adding it (over-engineering, unrequested scope) or silently ignoring it.
  - HTTPS/TLS is not yet in place anywhere in this stack (EC2 has no domain/SSL yet, per the "upcoming work" list) — until that's done, every login screen sends passwords in cleartext to the gateway. This is already a tracked, known, upcoming item, not a new finding, but worth restating here as the single most consequential item for login security specifically: everything audited in this entry is moot if the password itself is sent in the clear over the network.

- [x] Compute migration to EC2 — production-readiness gaps found and fixed before deploying code to the server. Started/closed 2026-09-12.

  **S3 access via EC2 IAM role, not static keys:** `docker-compose.prod.yml` had **zero** S3 configuration for `resource-service`/`resource-worker`/`practice-service`/`assessment-service` — dev's compose file hardcoded `AWS_STORAGE_BUCKET_NAME`/`AWS_S3_CDN_DOMAIN`/etc. directly in its `environment:` block, but prod's never got the equivalent. Separately, `storage.py`'s `_get_client()` in all 3 upload-handling services had a guard (`if not AWS_ACCESS_KEY_ID: raise RuntimeError(...)`) that would have actively blocked the "zero-code-change to an EC2 role" design its own docstring described — it required a static key even though the surrounding `boto3.client()` call never passes one explicitly.

  Fixed with the IAM role now attached to the instance (`spark-backend-ec2-role`, S3 List/Read/Write scoped to `spark-app-media-2026` only):
  - `docker-compose.prod.yml`: added `AWS_DEFAULT_REGION`/`AWS_STORAGE_BUCKET_NAME`/`AWS_S3_CDN_DOMAIN` (non-secret) to all 4 services — deliberately **not** `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`, so boto3 falls through to the instance role automatically.
  - `resource-service`/`practice-service`/`assessment-service`'s `core/settings.py`: removed `_require_env("AWS_ACCESS_KEY_ID")`/`_require_env("AWS_SECRET_ACCESS_KEY")` from the production-required block (kept `AWS_S3_CDN_DOMAIN` required).
  - Same 3 services' `core/storage.py`: guard now checks `AWS_STORAGE_BUCKET_NAME` instead — a real, credential-method-agnostic precondition — rather than a static key that production is now deliberately not setting.
  - Validated: `docker compose -f docker-compose.prod.yml config --quiet` passes.
  - **Still to do once the instance is confirmed working end-to-end:** deactivate and delete the exposed IAM user (`spark-app-s3-user`)'s access key in the AWS Console — it's no longer used by production once this deploys, closing the exposure recorded in Section 9/S3 migration entry above.

  **CORS — missing entirely, only surfaced by this same migration:** neither `django-cors-headers` nor any CORS middleware exists in any of the 7 services, and nginx sent no CORS headers either. This was never a problem in dev, where `nginx.dev.conf` proxies the `frontend` container through the same origin as the API (`map $host $frontend_backend`) — the browser only ever saw one origin. The architecture decided earlier in this same session ("frontend: Vercel / backend: AWS") makes these two genuinely different origins for the first time, and without a fix, every browser API call from the deployed frontend would be blocked by CORS before ever reaching the backend.

  Fixed at the gateway (`gateway/nginx.conf` only — `nginx.dev.conf` untouched, since dev still shares one origin and doesn't need this): added a `map $http_origin $cors_origin` allow-list (fails closed — anything not matched gets `""`, not a wildcard), `Access-Control-Allow-*` headers on every response, and a preflight `OPTIONS` short-circuit (204, no body) answered directly by nginx rather than forwarded to a Django service, none of which implement their own `OPTIONS` handler. Verified with a real `nginx -t` syntax check.

  - [ ] **Still open:** the allowed-origin match in `gateway/nginx.conf` is a placeholder (`https://spark-app.vercel.app`) that will not match anything until the real Vercel URL exists — deliberately left fail-closed rather than guessed at. Update it (and reload nginx) as soon as the frontend is actually deployed, or every request from the real frontend will be blocked the same way an unmatched origin is meant to be blocked.

- [x] Ported richer student exam-taking UI from a divergent, uncommitted local working copy (`D:\repos\spark-codebase`) — found to share real git history with this repo up to a common ancestor commit, then diverged with exam-UI work that was never merged back here (explains an earlier "why does the exam page still look like the old version" question — the answer was that the newer version existed only on a different machine's disk, uncommitted, never ported). Started/closed 2026-09-15.

  Ported into `frontend/src/app/students/assessments/[assignment_id]/page.tsx` and its `StudentLayout.tsx` dependency, verified read-only against the source (`D:\repos\spark-codebase` was never modified — confirmed via `git status`/`git log` showing identical state before and after):
  - Flag-for-review per question, with a 4-state question-number legend (Not Visited / Answered / Not Answered / Flagged) replacing the previous simple 2-state (answered/not) coloring.
  - "Clear response" to unselect an already-answered question.
  - A dedicated mobile hamburger menu for the question-number grid, and a separate mobile dropdown for section navigation — `StudentLayout` gained a `useExamMobileMenu()` context and an `examRightContent` prop for this, additive only (verified via diff that every non-exam-mode and desktop exam-mode code path is byte-identical to before).
  - Full-bleed side panels fixed to the true viewport edges (previously in-flow, padded).

  **Deliberately removed, not an oversight:** manual drag-to-resize on the Sections/Question-numbers side panels — the source copy's own later commits removed this on purpose ("2026-08-31, explicit request" per its code comments); confirmed with the user before proceeding rather than assumed, since the two versions had diverged in both directions (D: had features the live version didn't, and vice versa) and this specific one was a genuine tradeoff, not a strict improvement.

  **Verification:** types matched exactly between both repos for every model this page depends on (`StudentQuestion`, `StudentAssignmentListItem`, `StudentProfile`) — confirmed via diff before porting, not assumed. `tsc --noEmit` clean. One test file (`studentExamSectionsNavigator.test.tsx`) needed fixing: its `StudentLayout` mock predated the new `useExamMobileMenu` export, and two assertions needed `findByText` instead of `getByText` for a genuine async-timing characteristic of the new UI (the Sections panel's content populates one render tick after mount, via an effect that measures the panel's own DOM rect). Full frontend suite: 379/379 passing, twice consecutively, confirmed via isolated re-runs after ruling out unrelated resource-contention flakiness (21 background Docker containers competing with parallel Jest workers on this machine) as the cause of an initial noisy full-suite run.

  **Still open:** no live browser verification was done — would need a real student login with an actual assigned, startable exam session, which wasn't trivially available in this environment. Automated test coverage exercises the same rendering paths (entry gate, active question, flagging, sections navigator, mobile menu) with mocked data, but a real end-to-end visual check is still worth doing before this is considered fully proven. Also unresolved and out of scope for this entry: `D:\repos\spark-codebase` still holds ~150+ other uncommitted files (a full IT portal, department management, bulk import, and more) with no backup anywhere — a separate, larger data-loss risk untouched by this port.
