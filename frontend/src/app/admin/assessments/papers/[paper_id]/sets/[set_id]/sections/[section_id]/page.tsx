...
import type {
  QuestionPaper, QuestionSet, QuestionSection, AssessmentQuestion, AssessmentQuestionOption,
...
function QuestionEditorModal({ isOpen, onClose, question, sectionId, setId, onCreated, onUpdated }: {
  isOpen: boolean; onClose: () => void;
  question: AssessmentQuestion | null; // null = creating new
  sectionId: string;
  setId: string; // only used for the pre-creation image presign URL (set-scoped, quota is per-paper anyway)
  ...
  async function handleSave(): Promise<boolean> {
    ...
    try {
      ...
      if (!current) {
        const res = await api.post<ApiSuccess<AssessmentQuestion>>(`/assessments/admin/sections/${sectionId}/questions/`, payload);
...
// ── Main page ─────────────────────────────────────────────────────────────────
export default function AdminAssessmentSectionPage() {
  const { paper_id, set_id, section_id } = useParams<{ paper_id: string; set_id: string; section_id: string }>();
  ...
  const load = () => {
    api.get<ApiSuccess<SectionDetail>>(`/assessments/admin/sections/${section_id}/`)
      .then(res => setDetail(res.data.data))
      .catch(err => { toast.error(getErrorMessage(err)); router.push(`/admin/assessments/papers/${paper_id}/sets/${set_id}`); })
    ...
    <AdminLayout>
      <PageWrapper>
        ...
        <PageHeader
          ...
          subtitle={detail ? `${detail.paper.title} — ${detail.set.label} — ${detail.questions.length} question${detail.questions.length === 1 ? "" : "s"}` : undefined}
          backHref={`/admin/assessments/papers/${paper_id}/sets/${set_id}`}
          breadcrumb={
            <span className="flex items-center gap-1.5">
              ...
              <ChevronRight size={12} />
              <span className="cursor-pointer hover:underline" onClick={() => router.push(`/admin/assessments/papers/${paper_id}/sets/${set_id}`)}>
                {detail?.set.label ?? "..."}
              </span>
            </span>
          }
        ...
        {loading ? (
          ...
          <EmptyState icon={ListChecks} title="No questions yet" subtitle="Add a question to start building this section." />
        ) : (
          <div className="space-y-3">
