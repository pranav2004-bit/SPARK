...
// IT-exclusive CRUD (2026-08-19) — Admin's version of this page
// (admin/batch/page.tsx) is read-only: no add/edit/delete affordances,
// list/view calls only. IT owns all batch management.

interface BatchForm {
  batch_name: string;
}

export default function ITBatchPage() {
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {batches.map((batch) => (
              <div
                key={batch.id}
                onClick={() => router.push(`/it/batch/${batch.id}`)}
