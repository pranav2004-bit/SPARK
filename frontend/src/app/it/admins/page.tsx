...
import {
  Users, UserX, UserCheck, UserPlus, Upload,
...
// IT-exclusive CRUD (2026-08-19, phase 1) — same endpoints, same full
// capability as super-admin/admins/page.tsx, which was restricted to
// read-only the same day (phase 2). Kept as a near-exact mirror rather than
// a shared component since the two pages live under different layout
// patterns (ITLayout wrapper vs. Super Admin's file-based layout.tsx).
...
interface CreateAdminForm  { name?: string; email: string; department: string }
interface EditAdminForm    { name: string; department: string }
...
function ActionRow({ admin, onEdit, onResetDefault, onToggleActive, onDelete }: ActionRowProps) {
  return (
...
export default function ITAdminsPage() {
  ...
  function openEditModal(admin: AdminRecord) {
    editForm.reset({ name: admin.name, department: admin.department ?? "" });
    setEditTarget(admin);
  }

  async function onEditAdmin(data: EditAdminForm) {
    if (!editTarget) return;
    setEditLoading(true);
    try {
      const res = await api.patch<{ data: AdminRecord }>(`/auth/admins/${editTarget.id}/`, {
        name: data.name.trim(),
        department: data.department,
      });
  ...
  return (
    <ITLayout>
      <PageWrapper>
                ...
                {filteredAdmins.map((admin, idx) => (
                  <tr key={admin.id} style={{ borderBottom: idx < filteredAdmins.length - 1 ? "1px solid var(--color-border)" : "none" }}>
                    ...
                    <td className="px-5 py-4 text-sm" style={{ color: "var(--color-text-muted)" }}>
                      {admin.department || <span style={{ fontStyle: "italic" }}>Unassigned</span>}
                    </td>
        ...
        {/* ── Edit Modal ─────────────────────────────────────────────────────────── */}
        <Modal isOpen={!!editTarget} onClose={() => { setEditTarget(null); }} title="Edit Admin" maxWidth="sm">
          <form onSubmit={editForm.handleSubmit(onEditAdmin)}>
            <div className="space-y-4">
