...
class Command(BaseCommand):
    help = (
        "Rotate a super-admin's password and invalidate all existing tokens "
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            nargs="?",
            default=None,
            help="Email of the super-admin to rotate. Defaults to SUPERADMIN_EMAIL.",
        )

    def handle(self, *args, **options):
        email = options["email"] or settings.SUPERADMIN_EMAIL

        try:
            user = User.objects.get(email=email, role="super_admin")
        except User.DoesNotExist:
            raise CommandError(f"Super-admin '{email}' does not exist.")

        self.stdout.write(f"Rotating password for super-admin '{email}'")
        ...
        logger.warning(
            "SECURITY AUDIT: super-admin password rotated for email=%s — "
            "token_version %s -> %s (all prior tokens invalidated). "
            "Performed via rotate_superadmin_password management command.",
