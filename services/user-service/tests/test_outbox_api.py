"""
Tests for the Super Admin Dead-Letter Queue API.
...
Negative cases
  - List returns 403 for admin (not super_admin).
  ...
  - Retry returns 403 for admin.
  - Retry returns 401 for unauthenticated.
  - Health returns 403 for admin.
...
class TestDeadLetterListPositive:
    ...
    def test_returns_dead_letter_events(self, super_admin_client, db):
        from users.models import OutboxEvent
    ...
    def test_response_shape_matches_serializer(self, super_admin_client, db):
        ev = _make_event()
        resp = super_admin_client.get("/api/users/admin/outbox/dead-letters/")
        assert resp.status_code == 200
    ...
    def test_results_ordered_newest_first(self, super_admin_client, db):
        from users.models import OutboxEvent
    ...
    def test_pagination_is_applied(self, super_admin_client, db):
        for i in range(3):
            _make_event(student_id=f"STU{i:03d}")
        resp = super_admin_client.get("/api/users/admin/outbox/dead-letters/?page_size=2")
        assert resp.status_code == 200
        body = resp.json()
        assert body["count"] == 3
        assert len(body["results"]) == 2

    def test_empty_when_no_dead_letter_events(self, super_admin_client, db):
        resp = super_admin_client.get("/api/users/admin/outbox/dead-letters/")
        assert resp.status_code == 200
        body = resp.json()
        assert body["count"] == 0
...
class TestDeadLetterListNegative:
    ...
    def test_excludes_pending_events(self, super_admin_client, db):
        _make_event(status="pending", student_id="PENDING_STU", attempts=3)
        resp = super_admin_client.get("/api/users/admin/outbox/dead-letters/")
        assert resp.json()["count"] == 0

    def test_excludes_done_events(self, super_admin_client, db):
        _make_event(status="done", student_id="DONE_STU", attempts=1, last_error="")
        resp = super_admin_client.get("/api/users/admin/outbox/dead-letters/")
        assert resp.json()["count"] == 0

    def test_excludes_processing_events(self, super_admin_client, db):
...
class TestDeadLetterRetryPositive:
    ...
    def test_resets_status_to_pending(self, super_admin_client, db):
        from users.models import OutboxEvent
    ...
    def test_clears_retry_after(self, super_admin_client, db):
    ...
    def test_clears_last_error(self, super_admin_client, db):
    ...
    def test_response_contains_updated_event(self, super_admin_client, db):
        ev = _make_event(status="dead_letter")
        resp = super_admin_client.post(
            f"/api/users/admin/outbox/dead-letters/{ev.id}/retry/"
        )
    ...
    def test_retried_event_is_picked_up_by_worker(self, super_admin_client, db):
...
class TestDeadLetterRetryNegative:
    ...
    def test_returns_404_for_nonexistent_event(self, super_admin_client, db):
    ...
    def test_returns_404_for_pending_event(self, super_admin_client, db):
    ...
    def test_returns_404_for_done_event(self, super_admin_client, db):
    ...
    def test_returns_404_for_processing_event(self, super_admin_client, db):
...
class TestOutboxHealthPositive:
    ...
    def test_returns_all_zero_when_empty(self, super_admin_client, db):
        resp = super_admin_client.get("/api/users/admin/outbox/health/")
        assert resp.status_code == 200
        data = resp.json()["data"]
        assert data == {"pending": 0, "processing": 0, "done": 0, "dead_letter": 0}

    def test_counts_each_status_correctly(self, super_admin_client, db):
    ...
    def test_response_has_success_true(self, super_admin_client, db):
        resp = super_admin_client.get("/api/users/admin/outbox/health/")
